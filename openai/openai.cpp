...
cpr::Response OpenAI::completions(const std::string &prompt,
    ...
    return cpr::Post(cpr::Url{OpenAI_API::COMPLETIONS}, get_headers(),
                     cpr::Body{payload});
}
...
cpr::Response OpenAI::chat_completions(const std::vector<std::string> &messages,
    ...
    for (const auto &message : messages) {
        j["messages"].push_back({{"role", "user"}, {"content", message}});
    }
    ...
    return cpr::Post(cpr::Url{OpenAI_API::CHAT_COMPLETIONS}, get_headers(),
                     cpr::Body{payload});
}
